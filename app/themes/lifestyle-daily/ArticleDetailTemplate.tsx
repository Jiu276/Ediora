...
export default function LifestyleDailyArticleDetailTemplate({ article, config }: ArticleDetailTemplateProps) {
      ...
      <header style={{ background: config.colors.cardBackground, padding: '40px 0', borderBottom: `1px solid ${config.colors.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', textAlign: 'center' }}>
          <Link href="/" style={{ textDecoration: 'none' }}>
            <Title level={2} style={{ margin: 0, color: config.colors.text, fontWeight: 300, fontSize: 36, letterSpacing: '2px', fontFamily: 'serif' }}>
              Ablogia
            </Title>
          </Link>
        </div>
      ...
      <article style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          {article.featuredImage && (
            <div style={{ marginBottom: 40, borderRadius: 4, overflow: 'hidden' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            {article.category && (
              <Tag color={config.colors.primary} style={{ borderRadius: 0, border: 'none', padding: '4px 12px', fontSize: 12, marginBottom: 16 }}>
                {article.category.name}
              </Tag>
            )}
            <Title level={1} style={{ marginBottom: 24, color: config.colors.text, fontSize: 'clamp(28px, 5vw, 42px)', fontWeight: 400 }}>
              {article.title}
            </Title>
          </div>
