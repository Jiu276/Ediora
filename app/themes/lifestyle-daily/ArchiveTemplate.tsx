...
export default function LifestyleDailyArchiveTemplate({ loading, archiveGroups, yearStats, config }: ArchiveTemplateProps) {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: config.colors.background, padding: '60px 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}><Skeleton active paragraph={{ rows: 10 }} /></div>
      </div>
    )
  }
      ...
      <header style={{ background: config.colors.cardBackground, padding: '40px 0', borderBottom: `1px solid ${config.colors.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', textAlign: 'center' }}>
          <Link href="/" style={{ textDecoration: 'none' }}>
            <Title level={1} style={{ margin: 0, color: config.colors.text, fontWeight: 300, fontSize: 36, letterSpacing: '2px', fontFamily: 'serif' }}>
              Ablogia
            </Title>
          </Link>
        </div>
            ...
            <Card style={{ marginBottom: 40 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
                {Array.from(yearStats.entries()).sort((a, b) => b[0] - a[0]).map(([year, count]) => (
                  <div key={year} onClick={() => { const element = document.getElementById(`year-${year}`); element?.scrollIntoView({ behavior: 'smooth' }) }} style={{ padding: '16px 24px', border: `1px solid ${config.colors.border}`, borderRadius: 4, cursor: 'pointer', textAlign: 'center', minWidth: 100 }}>
