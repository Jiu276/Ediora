...
async function main() {
  ...
  try {
    ...
    console.log(
      ...
      JSON.stringify(content.slice(Math.max(0, content.length - 220))),
    )
    console.log('generated excerpt len:', excerpt.length)
    console.log('generated excerpt tail:', JSON.stringify(excerpt.slice(-220)))
    console.log('hasIncompleteImg:', content.includes('<img src=\\\\'))
  } finally {
    await p.$disconnect()
  }
}
...
main().catch((e) => {
  console.error('regenerate-direct error:', e?.message || e)
  process.exit(1)
})
